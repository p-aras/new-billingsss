...
const GatepassGenerator = ({ parties, gatepasses, onSubmit, onBack }) => {
  ...
  const [billDetails, setBillDetails] = useState({
    ...
    totalBora: 0,
    totalPolybags: 0,
    isByHand: false,  // New checkbox for by-hand
    byHandQuantity: 0,
    byHandPersonName: '',
    byPorter: false
  });
  ...
  const areVehicleDetailsRequired = () => {
    // If all selected bills are by-hand, vehicle details are NOT required
    if (isAllByHand()) return false;
    // If there's any bill that's NOT by-hand, vehicle details ARE required
    return true;
  };
...
const updateBillsWithGatepassInfo = async (selectedBills, gatepassNumber) => {
  ...
  const handleBillDetailsChange = (e) => {
    ...
    if (name === 'isByHand') {
      // When checkbox is toggled, reset by-hand fields if unchecked
      if (!checked) {
        setBillDetails(prev => ({
          ...prev,
          isByHand: checked,
...
const openBillDetailsModal = (bill) => {
  // Check if bill is already selected
  if (isBillSelected(bill['Bill Number'])) {
    alert(`Bill ${bill['Bill Number']} is already selected`);
    return;
  }
  
  // Calculate total quantity from bill
  let totalQuantity = 0;
  
  // Try to get from parsedBillData
  ...
  // Try to get from Bill Data JSON
  ...
  // Reset bill details form with isByHand checkbox unchecked by default
  setBillDetails({
    totalPetti: 0,
    totalBora: 0,
    totalPolybags: 0,
    isByHand: false,  // Checkbox unchecked by default
    byHandQuantity: totalQuantity, // Keep the value but it won't be used until checkbox is checked
    byHandPersonName: '',
    byPorter: false
  });
  
  // Make sure the bill has parsedBillData
  ...
  // Handle modal submit - UPDATED with isByHand checkbox logic
const handleBillDetailsSubmit = () => {
  // Validate - at least one item should be entered
  ...
  // If by-hand is checked, validate by-hand details
  if (billDetails.isByHand) {
    ...
    // Person name is now optional - removed validation
  }

  // Add additional details to the bill object while preserving parsed data
  const enhancedBill = {
    ...currentSelectedBill,
    additionalDetails: {
    ...
    // Ensure parsedBillData is preserved
...
const generatePDF = (gatepassInfo) => {
  ...
  // Check if this is a by-hand gatepass
 ...
 if (isByHandOnly) {
  // Show handling details instead of vehicle details
  ...
  doc.text(wrappedText, 60, y + 20);
    ...
    // Show vehicle details as before
  ...
  // DISPATCH DETAILS BOX - KEPT ON RIGHT SIDE
  ...
  // Headers with left alignment (removed align:center)
  ...
  // Helper function to get total quantity from bill data
  const getTotalQuantity = (bill) => {
    // Try to get from parsedBillData first (most reliable)
    if (bill.parsedBillData) {
      if (bill.parsedBillData.totalQuantity) {
        return bill.parsedBillData.totalQuantity;
      }
      // Also check if total quantity is in items sum
    ...
    // Try to get from Bill Data JSON string
    if (bill['Bill Data (JSON)']) {
      try {
        ...
        // Calculate from items if totalQuantity not present
    ...
    // Try to get from Total Quantity column if exists
  ...
  gatepassInfo.selectedBills.forEach((bill, index) => {
    ...
    // ALL DATA LEFT-ALIGNED (removed align:center)
  ...
  // Draw vertical lines for the 6 columns
  ...
  // Add additional details section for each bill
  doc.setFont("times", "bold");
  doc.setFontSize(11);
                ...
                {gatepassData.selectedBills.map((bill, index) => (
                  <div key={index} className="selected-bill-card">
                    <div className="selected-bill-info">
                      ...
                      {/* DISPLAY HANDLING DETAILS IN SELECTED BILLS */}
        ...
        <div className="bill-info">
          ...
          <div className="bill-info-row">
            <strong>Total Quantity:</strong> {
              (() => {
                // Calculate total quantity from bill data
